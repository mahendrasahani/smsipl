...
class Apis{
  ...
  async getMessageDetails(url,id) {
    const apiResponse = await apiService.MessageDetails(url+'/IntMessageManager/GetMessageDetails', id);
    return apiResponse;
  }
  ...
  async ProcessMessage(url,id,scode) {
    const apiResponse = await apiService.processMessage(url+'/IntMessageManager/ProcessMessage',id,scode);
    return apiResponse.data;
  }
