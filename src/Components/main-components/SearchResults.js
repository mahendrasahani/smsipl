...
const fetchMessage = async (start,end) => {
  try {
    setLoading(true);
    const apiResponse = await Apis.GetMessageList('http://dpw1.afrilogitech.com/api',start,end);
    ...
    <div className='main-container' style={!value?style1:null}>
      ...
      <div className='searchbox'>                             
        ...
        <div className='span-box'>
            <span>
              <p>From Date</p>
              <input type="date" value={startdate} min="2023-01-05" max="2024-01-05" onChange={(e)=>setstartDate(e.target.value)}/>
              </span>
            <span>
              <p>To Date</p>
              
              <input type="date"  
              min="2023-01-05" max="2024-01-05"
              value={enddate} 
        ...
        filteredItems.length!==0?
          ...
          {<Table messages={filteredItems} />}
           </div>
          
          :

        items.length===0?<h1 style={{textAlign:"center",height:"200px"}}>No data is found for specific date  Filters......</h1>:
        <div className='table-box'>
        { <Table messages={items}/> }
