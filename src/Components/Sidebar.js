...
const Sidebar = () => {
     ...
     // ---------------------------------------------------------------function to hide/show sidebar based on screen size
      ...
      useEffect(() => {
        const handleClickOutside = (event) => {
          if (sidebarRef.current && window.innerWidth < 768 && !sidebarRef.current.contains(event.target)) {
            dispatch(setHidden(!hidden));
          }
        };
  ...
  return (
    <aside className="main-sidebar sidebar-dark-primary elevation-4" ref={sidebarRef} style={{background:"white",width:"255px",transform:hidden?"translateX(-255px)":"translateX(0px)"}}>
      ...
      <nav className="mt-2">
        ...
        >
          <li className="nav-item" >
            <a href="/dashboard" className={`nav-link ${url == "dashboard" ? "active" : ""}`} style={{height:"41px"}}>
              <img src="img/home.png" className="img-responsive" />
              <p>Dashboard</p>
            </a>
          </li>
          <li className="nav-item">
            <a href="/messages" className={`nav-link ${url == "messages" ? "active" : ""}`} style={{height:"41px"}}>
              <img src="img/msg.png" className="img-responsive" />
              <p>Messages</p>
            </a>
          </li>
          <li className="nav-item">
            <a href="/users" className={`nav-link ${url == "users" ? "active" : ""}`} style={{height:"41px"}}>
